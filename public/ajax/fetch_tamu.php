...
try {
    ...
    // Base query
    $query = "
    SELECT id, tgl, nama, telp, email, alamat, tujuan, ttd, foto, jam
    FROM tb_tamu
    WHERE 1=1
    ";
    ...
    // Filter search - berdasarkan nama dan tujuan
    if ($search !== '') {
        $search = "%{$search}%";
        $query .= " AND (nama LIKE ? OR tujuan LIKE ?)";
        $params[] = $search;
        $params[] = $search;
    }
    ...
    $query .= " ORDER BY tgl DESC LIMIT $limit OFFSET $offset";
